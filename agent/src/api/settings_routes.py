...
def _read_settings_env_values() -> Dict[str, str]:
    ...
    # The desktop host injects DPAPI-decrypted credentials into the child
    # process. Overlay only the declared credential names so the settings API
    # can report/test them without ever writing them back to dotenv.
    if os.environ.get("VIBE_TRADING_DESKTOP_SECURE_CREDENTIALS") == "1":
        secret_names = {"TUSHARE_TOKEN"}
        secret_names.update(
...
def _sync_runtime_env(provider: LLMProviderOption, updates: Dict[str, str]) -> None:
    ...
    if provider.api_key_env:
        key_value = os.environ.get(provider.api_key_env, "")  # env-gate: dynamic provider api_key_env
    ...
    base_url = os.environ.get(provider.base_url_env, "")  # env-gate: dynamic provider base_url_env
    if base_url:
